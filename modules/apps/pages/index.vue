...
<template>
  <a-layout>
    ...
    <a-layout>
      <uiTemplatesCollapsable :nav-tree="nav">
        <slot />
      </uiTemplatesCollapsable>
...
import ratings from '../components/ratings/ratings.vue';
  ...
  setup(){
    ...
    return {nav, added_components, modalConfirm , modal, generatejson, jsoncontent}
...
generateJson(event) {
  const organisms = this.added_components.map((org,index) => {
    return {[`org${index+1}`]:org  }  })
 
  this.generatejson = true  
  this.jsoncontent = JSON.stringify(
    { "screenName": "SHOP_SERVICES","organisms": organisms }
